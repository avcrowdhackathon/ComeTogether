...
* @copyright Chen, Yi-Cyuan 2014-2017
  ...
  */
  ...
  (function () {
    ...
    const NODE_JS = !root.JS_SHA256_NO_NODE_JS && typeof process === 'object' && process.versions && process.versions.node;
    if (NODE_JS) {
      root = global;
    } else if (WEB_WORKER) {
      root = self;
    ...
    var nodeWrap = function (method, is224) {
      ...
      const algorithm = is224 ? 'sha224' : 'sha256';
    ...
    Sha256.prototype.update = function (message) {
      ...
      let code; let index = 0; let i; const { length } = message; const
        { blocks } = this;
    ...
    Sha256.prototype.hash = function () {
      ...
      let h = this.h7; const { blocks } = this; let j; let s0; let s1; let maj; let t1; let t2; let ch; let ab; let da; let cd; let
        bc;
    ...
    function HmacSha256(key, is224, sharedMemory) {
      let i; const
        type = typeof key;
      if (type === 'string') {
        const bytes = []; const { length } = key; let index = 0; let
          code;
        for (i = 0; i < length; ++i) {
